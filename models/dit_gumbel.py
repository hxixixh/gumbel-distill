...
class GumbelNoiseEmbedder(nn.Module):
    """Embeds gumbel noise into vector representations."""
    def __init__(self, num_classes, cond_size):
        super().__init__()
        self.linear = nn.Linear(num_classes, cond_size, bias=False)
        
    def forward(self, noise):
        """
        Args:
            labels: Tensor of shape (batch_size, num_classes) gumbel noise
        Returns:
            Tensor of shape (batch_size, cond_size) with embedded gumbel noise.
        """
...
class GumbelDIT(nn.Module, huggingface_hub.PyTorchModelHubMixin):
  def __init__(self, config, vocab_size: int):
    ...
    self.gumbel_noise_embed = GumbelNoiseEmbedder(vocab_size - 1, dim) # remove mask token from vocab size
    if self.adaLN == True:
    ...
    self.gumbel_inject_mode = config.model.gumbel_inject_mode
    assert self.gumbel_inject_mode in ['add', 'replace']
  ...
  def forward(self, indices, gumbel_noise, sigma, sample_mode=False, store_kv=False):
    x = self.vocab_embed(indices)
    # print(gumbel_noise.shape)
    x_gumbel = self.gumbel_noise_embed(F.softmax(gumbel_noise, dim=-1))
    
    mask_id = self.vocab_size - 1
    
    mask_loc = indices == mask_id
    cross_attn = hasattr(self, 'block_diff_mask')
    
    if cross_attn: 
      seq_len = gumbel_noise.shape[1] + 1
      indices_xt = indices[:, :seq_len]
      mask_loc = indices_xt == mask_id
      mask_loc = mask_loc[:, 1:].unsqueeze(-1)
      if self.gumbel_inject_mode == 'replace':
        target_slice = x[:, 1:seq_len]
        x[:, 1:seq_len] = torch.where(mask_loc, x_gumbel, target_slice)
      elif self.gumbel_inject_mode == 'add':
        x_gumbel_masked = x_gumbel * mask_loc
        x[:, 1:seq_len] = x[:, 1:seq_len] + x_gumbel_masked
    elif x_gumbel.shape[1] == x.shape[1] - 1:
      mask_loc = mask_loc[:, 1:].unsqueeze(-1)
      if self.gumbel_inject_mode == 'replace':
        target_slice = x[:, 1:]
        x[:, 1:] = torch.where(mask_loc, x_gumbel, target_slice)
      elif self.gumbel_inject_mode == 'add':
        x_gumbel_masked = x_gumbel * mask_loc
        x[:, 1:] = x[:, 1:] + x_gumbel_masked # the first token is bos token so no need to add gumbel noise
    elif x_gumbel.shape[1] == x.shape[1] - 2:
      mask_loc = mask_loc[:, 1:-1].unsqueeze(-1)
      if self.gumbel_inject_mode == 'replace':
        target_slice = x[:, 1:-1]
        x[:, 1:-1] = torch.where(mask_loc, x_gumbel, target_slice)
      elif self.gumbel_inject_mode == 'add':
        x_gumbel_masked = x_gumbel * mask_loc
        x[:, 1:-1] = x[:, 1:-1] + x_gumbel_masked
    else:
      raise ValueError(
        f'Gumbel noise shape {x_gumbel.shape} does not match input shape {x.shape}')
