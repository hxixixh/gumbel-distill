...
class Metrics:
  ...
  def record_generative_perplexity(
    ...
    for i in range(num_batches):
      ...
      all_tokens = torch.cat([batch.flatten() for batch in samples_batch])
      _, counts = torch.unique(all_tokens, return_counts=True, sorted=False)
      probabilities = counts.float() / counts.sum()
      total_entropy = torch.special.entr(probabilities).sum().item()
