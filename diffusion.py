...
class Diffusion(L.LightningModule):
  def __init__(
    ...
    elif self.config.algo.backbone == 'dit_gumbel':
      self.backbone = models.dit_gumbel.GumbelDIT(
        self.config, vocab_size=self.vocab_size)
  ...
  def _validate_configuration(self):
    ...
    assert self.config.algo.backbone in {
      'dit', 'dit_gumbel', 'dimamba', 'ar', 'hf_dit'}
    if self.config.algo.parameterization == 'ar':
      assert not self.config.algo.time_conditioning
    if self.config.sampling.kv_cache:
      assert self.config.algo.name in {'ar', 'bd3lm', 'bd3lm_gumbel'}
    ...
    if self.config.model.attn_backend == 'flex':
      assert self.config.algo.name in ['bd3lm', 'bd3lm_gumbel'], 'Custom FlexAttention mask only supported for BD3LM.'
  ...
  def _mtp_parameterization(self, logits, xt):
    return self._subs_parameterization(logits, xt)
  ...
  def _process_sigma(self, sigma):
    ...
    if self.parameterization in ['ar']:
      return None
    if self.parameterization in ['mtp', 'mtp_gumbel']:
      return sigma
    assert sigma.ndim == 2
  ...
  def forward(self, x, sigma, gumbel=None, sample_mode=False, store_kv=False):
    """Returns log score."""
    ...
    with torch.amp.autocast('cuda', dtype=torch.float32):
      ...
      elif self.config.algo.name == 'bd3lm_gumbel':
        logits = self.backbone(x, gumbel, sigma,
                              store_kv=store_kv,
                              sample_mode=sample_mode)
      ...
      elif self.config.algo.name == 'mdlm_gumbel':
        logits = self.backbone(x, gumbel, sigma)
      elif self.config.algo.name == 'mtp':
        logits = self.backbone(x, sigma)
      elif self.config.algo.name == 'mtp_gumbel':
        logits = self.backbone(x, gumbel, sigma)
      else:
        logits = self.backbone(x, sigma)
    ...
    if self.parameterization == 'subs':
      return self._subs_parameterization(logits=logits,
                                      xt=x)
    elif self.parameterization == 'mtp':
      return self._mtp_parameterization(logits=logits,
                                        xt=x)
  ...
  def training_step(self, batch, batch_idx):
    del batch_idx
    if self.config.data.train == 'gpt2_distill':
      gumbel_seeds = batch['gumbel_seeds']
    else:
      gumbel_seeds = None
    losses = self._loss(batch['input_ids'],
                        batch['attention_mask'],
                        gumbel_seeds=gumbel_seeds,
                        )
    ...
    self.log(name='trainer/loss',
             ...
             on_epoch=False,
             sync_dist=True)
    self.log(name='trainer/token_nll',
             value=losses.token_nll.item(),
             on_step=True,
             on_epoch=False,
             sync_dist=True)
    return losses.loss
      ...
      for noise_clip_start in self.metrics.valid_vars.keys():
        ...
        if self._check_val_sampling_intvl(sampling_eps_min, sampling_eps_max) == True:
          ...
          losses = Loss(
            ...
            token_nll=losses_clip.token_nll.clone(),
  ...
  @torch.no_grad()
  def _ddpm_caching_update(self, x, t, dt, p_x0=None, gumbel=None):
    ...
    if self.config.sampling.first_hitting:
      ...
      k = int(self.block_size * dt)
      x_block = _sample_categorical(p_x0)
      rand_scores = torch.rand(x_block.shape, device=x.device)
      masked_positions = (x[:, -self.block_size:] == self.mask_index)
      rand_scores.masked_fill_(~masked_positions, -1.0)      
      _, ind = torch.topk(rand_scores, k, dim=-1)
      mask = torch.zeros_like(x_block, dtype=x_block.dtype)
    ...
    if self.config.sampling.kv_cache and self.mask_index not in x_block:
      _ = self.forward(x_block, sigma_t, gumbel=gumbel, sample_mode=True, store_kv=True)
  ...
  def _sample(
    ...
    samples_original=None, samples_partial_masked=None, gumbel_seeds=None):
    ...
    if self.sampler == 'semi_ar':
      for _ in range(self.config.sampling.num_sample_batches):
        ...
        while sample_i is None:
          num_tries += 1
          if 'gumbel' in self.config.algo.name:
            sample_i, nfes = self._semi_ar_sampler_w_gumbel(
    ...
    elif self.sampler == 'ddpm_caching':
      nfes = num_steps
      for i in range(self.config.sampling.num_sample_batches):
        if samples_partial_masked is not None:
          samples_original_i = samples_original[i].reshape(1, -1).to(self.device)
          samples_partial_masked_i = samples_partial_masked[i].reshape(1, -1).to(self.device)
        else: 
          samples_original_i = None
          samples_partial_masked_i = None
          
        if self.config.algo.name in ['mdlm_gumbel', 'bd3lm_gumbel']:
          sample_i = self._ddpm_caching_sampler_w_gumbel(
            n_samples=batch_size_per_gpu,
            num_steps=num_steps,
            seqlen=seqlen,
            eps=eps,
            samples_original=samples_original_i,
            samples_partial_masked=samples_partial_masked_i, 
            # gumbel_seeds=gumbel_seeds,
            sample_idx=i)
        else:
          sample_i = self._ddpm_caching_sampler(
            n_samples=batch_size_per_gpu,
            num_steps=num_steps,
            seqlen=seqlen,
            eps=eps,
      ...
      for _ in range(self.config.sampling.num_sample_batches):
        ...
        self.metrics.nfes.update(nfes)
        self.metrics.gen_nfes.append(nfes)
    elif self.sampler == 'mtp':
      if self.config.algo.name in ['mtp_gumbel']:
        sampler = self._mtp_sampler_w_gumbel
      else:
        sampler = self._mtp_sampler
      ...
      return self.tokenizer.batch_decode(samples), losses
  ...
  def restore_model_and_partial_sample(self, num_steps, eps=1e-5, seqlen=None, x0s=None, gumbel_seeds=None):
    """Generate samples from the model."""
    if self.ema:  
      self.ema.store(self._get_parameters())
      self.ema.copy_to(self._get_parameters())
    ...
    # num_masked_tokens = self.config.sampling.num_masked_tokens
    
    x0s_partial_masked = []
    
    for k in range(self.config.sampling.num_sample_batches):
      x0 = x0s[k]
      
      if self.config.algo.sampler == 'mtp':
        num_masked_tokens = self.config.algo.num_masked_tokens
        mask_index = torch.arange(self.config.model.length - num_masked_tokens,
                                  self.config.model.length,
                                  device=x0.device, dtype=torch.int64)
        mask_index = mask_index.unsqueeze(0)
      else:
        # mask_index = torch.tensor(
        #   [[10, 11, 12, 13, 14, 15]], 
        #   device=x0.device, dtype=torch.int64)
        num_masked_tokens = 10
        mask_index = torch.arange(self.config.model.length - num_masked_tokens,
                                  self.config.model.length,
                                  device=x0.device, dtype=torch.int64)
        mask_index = mask_index.unsqueeze(0)
        
      x0_partial_masked = x0.clone()
      mask_tokens = torch.full_like(mask_index, self.mask_index)
      x0_partial_masked.scatter_(1, mask_index, mask_tokens)
      
      x0s_partial_masked.append(x0_partial_masked)
    
    x0s = torch.cat(x0s, dim=0)
    x0s_partial_masked = torch.cat(x0s_partial_masked, dim=0)
    
    samples, losses = self._sample(
      seqlen=seqlen,
      batch_size_per_gpu=self.config.loader.eval_batch_size,
      num_steps=num_steps,
      eps=eps,
      samples_original=x0s,
      samples_partial_masked=x0s_partial_masked, 
      gumbel_seeds=gumbel_seeds)
    self.metrics.record_generative_perplexity(
      samples,
      self.config.model.length,
    ...
    original_samples = self.tokenizer.batch_decode(x0s)
    samples_masked = self.tokenizer.batch_decode(x0s_partial_masked)

    return samples, original_samples, samples_masked, losses

  def get_score(self, x, sigma, gumbel=None):
    model_output = self.forward(x, sigma, gumbel=gumbel).to(torch.float64)
  ...
  def _analytic_update(self, x, t, dt, gumbel=None):
    sigma_t = self._sigma_from_p(self.noise(t)[1])
    sigma_s = self._sigma_from_p(self.noise(t - dt)[1])
    dsigma = sigma_t - sigma_s
    score = self.get_score(x, sigma_t, gumbel=gumbel)
    stag_score = self._staggered_score(score, dsigma)
    probs = stag_score * self._transp_transition(x, dsigma)
    return _sample_categorical(probs)


  def _denoiser_update(self, x, t, gumbel=None):
    sigma = self._sigma_from_p(self.noise(t)[1])
    score = self.get_score(x, sigma, gumbel=gumbel)
  ...
  def _forward_mtp(self, x0, gumbel_seeds=None):
    x_masked = x0.clone()
    n = self.config.algo.num_masked_tokens
    x_masked[:, -n:] = self.mask_index
    x_input = x_masked
    
    sigma = torch.zeros(x0.shape[0], device=self.device)
    
    if self.config.algo.name == 'mtp_gumbel':
      if gumbel_seeds is None:
        with torch.no_grad():
          teacher_out = self.teacher_model(x0[:, :-1])
          teacher_logits = teacher_out.logits
          # prob_p = teacher_logits.softmax(dim=-1)
          # log_p = torch.log(prob_p)
          log_p = F.log_softmax(teacher_logits, dim=-1)
          
          x0_shift_by_1 = x0[:, 1:]
          gumbel = utils.get_posterior_gumbel(log_p.reshape(-1, log_p.size(-1)), x0_shift_by_1.reshape(-1)).reshape(
              log_p.shape
          ) # [B, 1023, vocab_size]
          # x0: [B, 1024]
        model_output = self.forward(x_input, gumbel=gumbel, sigma=sigma)
      else:
        bsz = x0.shape[0]
        
        num_pred_tokens = x0.shape[1] - 2 # if we ignore the bos and eos tokens
        batch_gumbel_noise = torch.empty(
            (bsz, num_pred_tokens, self.tokenizer.vocab_size),
            dtype=torch.float32, device=self.device # Use float32 for Gumbel noise
        )
        
        num_pred_tokens = 128
        for j in range(bsz):
          item_seed = gumbel_seeds[j].item()
          generator = torch.Generator(device=self.device).manual_seed(item_seed)
          U = torch.rand(
                    size=(num_pred_tokens, self.tokenizer.vocab_size), 
                    device=self.device, 
                    generator=generator)
          gumbel_noise = -torch.log(-torch.log(U.clamp(min=1e-20)))
          gumbel_noise = gumbel_noise[:x0.shape[1] - 2] # TODO: ugly here
          batch_gumbel_noise[j] = gumbel_noise
        gumbel = batch_gumbel_noise
        self.backbone.eval()
        model_output = self.forward(x_input, sigma=sigma, gumbel=gumbel)
    else: 
  ...
  def _forward_pass_diffusion(self, x0, gumbel_seeds=None, t=None, sampling_eps_min=None, sampling_eps_max=None):
    if t is None:
    ...
    if self.config.algo.name in ['bd3lm_gumbel', 'mdlm_gumbel']:
      if gumbel_seeds is None:
        # TODO: remove the mask token since we don't need to use it
        with torch.no_grad():
          teacher_out = self.teacher_model(x0)
          teacher_logits = teacher_out.logits
          prob_p = teacher_logits.softmax(dim=-1)
          
          log_p = torch.log(prob_p)
          log_p = log_p[:, :-1] # remove the last prediction because we don't have the corresponding token for the last logit
        ...
        if self.config.algo.use_gaussian:
          gumbel = torch.randn_like(gumbel)
        model_output = self.forward(x_input, gumbel=gumbel, sigma=sigma)
      else:
        bsz = x0.shape[0]
        num_pred_tokens = x0.shape[1] - 2 # if we ignore the bos and eos tokens
        ...
        if self.config.algo.use_gaussian:
          gumbel = torch.randn_like(gumbel)
        elif self.config.algo.use_uniform:
          gumbel = U
        model_output = self.forward(x_input, gumbel=gumbel, sigma=sigma)
    else: 
      model_output = self.forward(x_input, sigma=sigma)
  ...
  def _loss(self, x0, attention_mask, gumbel_seeds=None, t=None, sampling_eps_min=None, sampling_eps_max=None):
    ...
    if self.parameterization == 'ar':
      ...
      loss = self._forward_mtp(
        input_tokens, 
        gumbel_seeds=gumbel_seeds)
    else:
      loss_dict = self._forward_pass_diffusion(
        input_tokens,
        gumbel_seeds=gumbel_seeds,
        sampling_eps_min=sampling_eps_min,
        sampling_eps_max=sampling_eps_max,)
  ...
  @torch.no_grad()
  def _ddpm_caching_update_2(self, x, t, dt, p_x0=None, gumbel=None):
    _, move_chance_t = self.noise(t)
    _, move_chance_s = self.noise(t - dt)
    sigma_t = self._sigma_from_p(move_chance_t)
    move_chance_t = move_chance_t[:, None]
  ...
  @torch.no_grad
  def _ddpm_caching_sampler(
    self, n_samples, num_steps, seqlen, eps=1e-5, samples_partial_masked=None, sample_idx=0): 
    
    if samples_partial_masked is None:
      x = self._sample_prior(
        n_samples,
        seqlen).to(self.device)
      x[:, 0] = self.tokenizer.bos_token_id
    else:
      x = samples_partial_masked
    ...
    for i in tqdm(range(num_steps), desc='step'):
      if self.mask_index not in x:
          break
      
      # faster (equivalent) sampler from zheng et al (2025)
      if self.config.sampling.first_hitting:
        u = np.random.rand()
        num_masked = (x == self.mask_index).sum(-1).item()
        t *= u**(1 / num_masked)
      elif not self.config.sampling.first_hitting:
        t = timesteps[i]
    
      t = t * torch.ones((n_samples,1), dtype=self.dtype, device=self.device)
      p_x0_cache, x_next = self._ddpm_caching_update_2(
  ...
  @torch.no_grad
  def _ddpm_caching_sampler_w_gumbel(
    self, n_samples, num_steps, seqlen, eps=1e-5, samples_original=None, samples_partial_masked=None, sample_idx=0): 
    ...
    if self.config.data.train == 'gpt2_distill':
      ...
      # self.vocab_size - 1 to remove mask token probability
      
      # debug the gumbel sampling, use training gumbel seeds
      # item_seed = 
      # if self.config.sampling.use_train_gumbel:
      #   seeds_path = self.config.data.gumbel_seeds_path
      #   seeds_tensor = torch.load(seeds_path, map_location='cpu')

      #   item_seed = seeds_tensor[sample_idx].item()
      #   item_seed = 2926616133
      # else:
      #   item_seed = random.randint(0, 2**32 - 1)
      # generator = torch.Generator(device=self.device).manual_seed(item_seed)
      # U = torch.rand(
      #               size=(128, self.vocab_size - 1), 
      #               device=self.device, 
      #               generator=generator)
      # gumbel_noise = -torch.log(-torch.log(U.clamp(min=1e-20)))
      
      # gumbel_noise = gumbel_noise[None, :x.shape[1] - 2] # TODO: ugly here
      
      gumbel = utils.sample_gumbel(
          (n_samples, seqlen - 2, self.vocab_size - 1), 
          temperature=self.config.sampling.gumbel_temperature,
          device=x.device, return_uniform=self.config.algo.use_uniform)
      if self.config.algo.use_gaussian:
          gumbel = torch.randn_like(gumbel)
    else:
      # if self.config.sampling.use_train_gumbel:
      ...
      #                 generator=generator)
      #   gumbel_noise = -torch.log(-torch.log(U.clamp(min=1e-20)))
      #   gumbel_noise = gumbel_noise[None, :x.shape[1] - 1]
        
      # remove bos token
      # self.vocab_size - 1 to remove mask token probability
      gumbel = utils.sample_gumbel(
        (n_samples, seqlen - 1, self.vocab_size - 1), 
        temperature=self.config.sampling.gumbel_temperature,
        device=x.device, return_uniform=self.config.algo.use_uniform)
      if self.config.algo.use_gaussian:
          gumbel = torch.randn_like(gumbel)
    ...
    for i in tqdm(range(num_steps), desc='step'):
      ...
      if self.config.sampling.resample_gumbel:
        if self.config.data.train == 'gpt2_distill':
          gumbel = utils.sample_gumbel(
            (n_samples, seqlen - 2, self.vocab_size - 1), 
            temperature=self.config.sampling.gumbel_temperature,
      ...
      p_x0_cache, x_next = self._ddpm_caching_update_2(
        x, t, dt, p_x0=p_x0_cache, gumbel=gumbel)
      if (not torch.allclose(x_next, x) or self.time_conditioning):
          # Disable caching
          p_x0_cache = None
      x = x_next
    ...
    if self.config.sampling.noise_removal:
      t = timesteps[-1] * torch.ones(x.shape[0], 1,
                                    device=self.device)
      unet_conditioning = self.noise(t)[0]
      x = self.forward(x, unet_conditioning, gumbel=gumbel).argmax(dim=-1)
  ...
  def _semi_ar_sampler_w_gumbel(
    ...
    if self.config.data.train == 'gpt2_distill':
      gumbel = utils.sample_gumbel(
        (n_samples, context_size - 2, self.vocab_size - 1), 
        temperature=self.config.sampling.gumbel_temperature,
        device=self.device)
    else: 
      gumbel = utils.sample_gumbel(
        (n_samples, context_size - 1, self.vocab_size - 1), 
        temperature=self.config.sampling.gumbel_temperature,
        device=self.device)
    ...
    for stride_num in tqdm(range(num_strides)):
      ...
      for i in tqdm(range(num_steps), desc="Sampling"):
        ...
        # print("stride_num", stride_num, 'i', i, 'idx', fwd_idx[:-1], gumbel.shape, gumbel[:, fwd_idx[:-1]].shape)
        p_x0_cache, x_next = self._ddpm_caching_update(
            x=x_accum[:, fwd_idx],
            t=t * ones,
            dt=dt,
            p_x0=p_x0_cache,
            gumbel=gumbel[:, fwd_idx[:-1]])
        if p_x0_cache is None:
          sampling_steps += 1
        x_accum[:, fwd_idx] = x_next
  ...
  @torch.no_grad
  def _mtp_sampler(self, n_samples, seqlen=None, eps=1e-5, 
                   samples_original=None, samples_partial_masked=None, gumbel_seeds=None, sample_idx=0): 
    """Sample from the model using multi token prediction."""
    assert samples_partial_masked is not None, \
      'samples_partial_masked must be provided for MTP sampling.'
    ...
    sigma = torch.zeros(x.shape[0], device=self.device)
    p_x0 = self.forward(x, sigma, sample_mode=True).to(torch.float64)
    log_p_theta = torch.gather(
      input=p_x0.clone(),
      dim=-1,
      index=samples_original[:, :, None]).squeeze(-1)
    loss = - log_p_theta
    p_x0 = p_x0.exp()
    p_x0 = self._nucleus_sample(p_x0)
    x_block = _sample_categorical(p_x0)
    ...
    copy_flag = (x[:, -self.block_size:] != self.mask_index).to(x.dtype)
    x =  copy_flag * x[:, -self.block_size:] + (1 - copy_flag) * x_block
    
    return x, loss
  
  @torch.no_grad
  def _mtp_sampler_w_gumbel(self, n_samples, seqlen=None, eps=1e-5, 
                            samples_original=None, samples_partial_masked=None, gumbel_seeds=None, sample_idx=0): 
    """Sample from the model using multi token prediction."""
    assert samples_partial_masked is not None, \
      'samples_partial_masked must be provided for MTP sampling.'
    ...
    if self.config.data.train == 'gpt2_distill':
      if self.config.sampling.use_train_gumbel:
        item_seed = gumbel_seeds[sample_idx].item()
      else:
        item_seed = random.randint(0, 2**32 - 1)
      generator = torch.Generator(device=self.device).manual_seed(item_seed)
      U = torch.rand(
    ...
    sigma = torch.zeros(x.shape[0], device=self.device)
    p_x0 = self.forward(x, sigma, gumbel=gumbel, sample_mode=True).to(torch.float64)
    log_p_theta = torch.gather(
      input=p_x0,
      dim=-1,
      index=samples_original[:, :, None]).squeeze(-1)
